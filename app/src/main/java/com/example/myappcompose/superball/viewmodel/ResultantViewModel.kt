    ...
    fun getGameList() {
        viewModelScope.launch {
            ...
            if (!list.isNullOrEmpty()) {
                ...
                val groupedByGameName = list.groupBy { it.gameName }
                val ss = groupedByGameName.map { (gameName, subList) ->
                    val selected = subList.toList() // Convert to immutable list
                    ResultantModel(gameName, selected)
                }
                Log.d("TAG", "getGameList: " + groupedByGameName.toString())
                Log.d("TAG", "getGameList: " + ss.toString())
